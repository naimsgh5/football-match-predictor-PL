...
def betting_notes(p_home: float, p_draw: float, p_away: float, btts_probs: dict, ou_probs: dict) -> list[str]:
    """A few textual observations on markets where one side stands out clearly.
    Purely informational: never recommends a specific bet, just what the model says."""
    notes = []
    ...
    if gap >= 0.20:
        notes.append(f"Result: '{best_result}' is clearly favored ({best_p*100:.0f}%).")
    elif gap >= 0.08:
        notes.append(f"Result: '{best_result}' is slightly favored ({best_p*100:.0f}%), not settled.")
    else:
        notes.append("Result: no outcome stands out clearly, an uncertain match.")

    if btts_probs["yes"] >= 0.58:
        notes.append(f"BTTS: both teams are likely to score (Yes {btts_probs['yes']*100:.0f}%).")
    elif btts_probs["no"] >= 0.58:
        notes.append(f"BTTS: at least one team may fail to score (No {btts_probs['no']*100:.0f}%).")
    else:
        notes.append("BTTS: no clear tendency.")

    line_25 = ou_probs.get(2.5)
    if line_25:
        if line_25["over"] >= 0.58:
            notes.append(f"Goals: leaning towards a high-scoring match (Over 2.5 {line_25['over']*100:.0f}%).")
        elif line_25["under"] >= 0.58:
            notes.append(f"Goals: leaning towards a tight, low-scoring match (Under 2.5 {line_25['under']*100:.0f}%).")
        else:
            notes.append("Goals: no clear tendency around 2.5.")

    notes.append(
        "For informational purposes only: simplified statistical model, no guarantee of "
        "outcome. Bet responsibly, only what you can afford to lose."
    )
    return notes
