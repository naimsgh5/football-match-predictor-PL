...
- [x] **M4 — MLP (PyTorch)**
  ...
  - Poisson goal model (`src/models/markets.py`) improved along 2 axes:
    1. **Team attack/defense ratings** (`src/features/team_ratings.py::fit_team_ratings`): a Dixon-Coles/Maher-style joint Poisson MLE fit, replacing what was briefly a simple venue-specific rolling average (itself replacing the original venue-mixed average + flat `HOME_ADVANTAGE = 1.10` multiplier). Fits 4 log-scale parameters per team — `attack_home`, `defense_home`, `attack_away`, `defense_away` — over every match at once, so each team's rating is adjusted for the strength of the opponents it actually faced (a raw average can't tell a genuinely strong team from one that just played weak opposition; this is the same problem `quality_form_diff` solves for `form_diff` on the 1X2 side). Splitting attack/defense by venue (beyond the original 1997 paper's single rating + shared home-advantage constant) lets the model directly represent a team that's hard to beat at home without necessarily being high-scoring there. Time-decayed (`HALF_LIFE_DAYS = 365`, a match this old counts half as much) since a team's strength isn't stable over 5 seasons of transfers, and ridge-regularized (`RIDGE_LAMBDA = 0.01`, shrinks towards a prior worth `DEFAULT_GOALS_AVG` expected goals) both to control overfitting on ~19-23 home matches/team/season and to make the fit identifiable (attack/defense only ever appear as a sum). A team with zero history (freshly promoted) falls back to the shared prior automatically. Purely a Poisson-model input — not in `FEATURE_COLUMNS`, doesn't touch the trained classifiers.
    2. **Dixon-Coles low-score correction** (Dixon & Coles, 1997), unchanged in kind but recalibrated for the new lambdas: the 4 low-scoring cells (0-0, 1-0, 0-1, 1-1) of the goal matrix get a correction factor (`DIXON_COLES_RHO = -0.030`, fit by maximum likelihood on PL's own historical low-scoring matches) — plain independent Poisson is known to be biased there specifically, since goals in a low-scoring match aren't perfectly independent event-by-event. This is a separate, complementary correction to the ratings above: it fixes the independence *assumption* between the two teams' goal counts, not the expected values themselves.
    - `tests/test_team_ratings.py`: synthetic-league tests (a known-strong/known-weak team pair) confirming the fit recovers the right ranking, handles unrated teams, and weighs recent matches more than old ones
    - `calibrate_rho.py` (one-off script, not checked in) was used to compute the calibrated rho from `data/raw/premier_league_results.csv`
    - JS port re-verified against the same shadow-check methodology after this change
  - **The goal market now reacts to the manual post-hoc context** (injuries, lineup, rest days, standings, stakes/derby, odds) — previously it was frozen at the historical-ratings estimate even when the 1X2 models clearly moved in response to the same inputs (e.g. an injured striker lowered the home win probability but left "expected goals" completely untouched). `_apply_all_goal_adjustments()` mirrors `_apply_all_adjustments()` (same inputs, same order) but reshapes each signal as a **multiplicative** shift on `lambda_home`/`lambda_away` instead of an additive shift on the 1X2 probabilities; a new `GOALS_*` family of impact constants controls the magnitude, chosen to be comparable in order of magnitude to the existing probability-impact constants (not independently calibrated against real goal outcomes — same heuristic status as the rest of this adjustment system)
  - **Bug found and fixed while wiring the above**: `_apply_injuries()` had `str_home`/`str_away` swapped — since `injury_strength()` returns a *strength factor* (1.0 = full squad, lower = weakened, not a "severity" score), the sign was inverted, so a team's **own** injury was silently **increasing** its win probability (verified: Bayern Munich losing Harry Kane raised their win probability from 82.3% to 82.7%). Pre-existing bug, not introduced by this change — caught only because auditing the injury adjustment's exact formula was necessary to port it to the goal market correctly. Fixed in both `predict.py` and the WebUI's JS port (same bug, faithfully transcribed). `tests/test_predict_adjustments.py` (new) locks in the correct direction for both the 1X2 and goal-market injury/lineup adjustments so this can't silently regress again
