...
- [x] **M4 — MLP (PyTorch)**
  ...
  - Poisson goal model (`src/models/markets.py`) improved along 3 axes:
    1. **Venue-specific goals** (`rolling_stats.py::add_venue_goals_features`): `expected_goals()` now uses each team's own home-scored/home-conceded vs away-scored/away-conceded averages, instead of goals pooled across both venues (still used unchanged for `attack_diff`/`defense_diff`, the trained classifiers' features)
    2. **Calibrated `HOME_ADVANTAGE`**: measured on the actual data (average home goals ÷ model-predicted average with no multiplier) instead of a hardcoded `1.10`. Comes out to `0.996` — once goals are already venue-specific, there's no separate home-field effect left to compensate for; the old `1.10` was almost entirely correcting for the lack of a venue split (empirically it matches: the venue-mixed formula would have needed `1.095`)
    3. **Dixon-Coles low-score correction** (Dixon & Coles, 1997): the 4 low-scoring cells (0-0, 1-0, 0-1, 1-1) of the goal matrix get a small correction factor (`DIXON_COLES_RHO = -0.026`, fit by maximum likelihood on PL's own historical low-scoring matches) — plain independent Poisson is known to be biased there specifically, since goals in a low-scoring match aren't perfectly independent event-by-event
    - `calibrate_markets.py` (one-off script, not checked in) was used to compute both calibrated constants from `data/raw/premier_league_results.csv`
